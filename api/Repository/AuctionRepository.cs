    ...
    public class AuctionRepository : IAuctionRepository
    {
        ...
        public async Task<List<Auction>> GetAllAuctionsAsync(QueryObject query)
        {
            var result = _context.Auctions.Include(a => a.Biddings).ThenInclude(b => b.Bidder).ThenInclude(b => b.Business)
                                            ...
                                            .Where(a => a.AuctionStatus == AuctionStatus.Approved || a.AuctionStatus == AuctionStatus.Ended || a.AuctionStatus == AuctionStatus.Started)
                                            .AsQueryable();
        ...
        public async Task<List<Auction>> GetAllAuctionsForAdminAsync(QueryObject query)
        {
            var result = _context.Auctions.Include(a => a.Biddings).ThenInclude(b => b.Bidder).ThenInclude(b => b.Business)
                                            .Include(a => a.Biddings).ThenInclude(b => b.Bidder).ThenInclude(b => b.Individual)
                                            .Include(a => a.Seller).ThenInclude(s => s.Business).Include(a => a.Category)
        ...
        {
            return await _context.Auctions.Include(a => a.Category)
                                            .Include(a => a.watches)
                                            .Where(a => a.SellerId == userId).ToListAsync();
        }
